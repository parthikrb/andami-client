...
import editTeamFormColumn from "./edit-team-form-column";
...
const EditTeamForm = ({ teamId }: EditTeamFormProps) => {
  ...
  const team: any = queryClient.getQueryData([API_KEYS.TEAM, teamId]) as any;

  const availableRoles: any[] = queryClient.getQueryData([
    API_KEYS.ROLES,
  ]) as any[];

  const allUsers: any[] = queryClient.getQueryData([API_KEYS.USERS]) as any[];

  const [unAssignedUsers, setUnAssignedUsers] = useState<any[]>([]);

  const updateTeamMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => updateTeam(teamId, data),
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    setUnAssignedUsers(
      allUsers.filter(
        (user) => !team.users.map((user: any) => user.id).includes(user.id)
      )
    );
  }, [allUsers, team]);
              ...
              <FormItem>
                ...
                <FormControl>
                  <DataTable
                    ...
                    optionalComponent={
                      <AddUserToTeam users={unAssignedUsers} form={form} />
                    }
                  />
                </FormControl>
                <FormMessage />
